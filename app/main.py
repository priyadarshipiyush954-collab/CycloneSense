...
@app.get("/health")
def health(): return {"status":"ok","service":"cyclonesense-api"}

@app.get("/patterns")
def patterns(): return {"patterns":PATTERNS}
...
async def predict_pattern(file: UploadFile=File(...)):
    ...
    try: label,confidence=classify_demo(data)
    except Exception as e: raise HTTPException(400,f"Invalid image: {e}")
    return {"pattern":label,"confidence":confidence,"model":"demo-morphology-baseline","disclaimer":"Prototype result; not an official warning."}
...
def predict_forecast(req:ForecastRequest):
    ...
    return ForecastResponse(next_lat=x[0],next_lon=x[1],predicted_wind_kts=x[2],intensity_class=x[3],confidence=x[4])
